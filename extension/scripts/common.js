(function () {
  ...
  var SirsiCatalog = function (extension, request) {
    ...
    this.mediaType = 'book'
    this.baseUrl = 'https://greencommons.net/' //'https://catalog.dclibrary.org/'
	var t = encodeURI(request.title.toLowerCase());
    this.searchUrls = [
		this.baseUrl + 'api/v1/search?q=' + t + '&filters[resource_types]=books&filters[model_types]=resources',
		this.baseUrl + 'api/v1/search?q=' + t + '&filters[resource_types]=books&filters[model_types]=resources'
    ]
  ...
  SirsiCatalog.prototype.parseAvailability = function (data) {
    ...
    console.log(this.author.toLowerCase())
    debugger;
      
    let availabilityData

	var max_search = Math.min(5, data.data.length);
    for (let i = 0; i < max_search; i++) {
    	if (data.data[i].attributes['title'] == this.title){
    		this.copies = 1
    		this.available = 1
	    	this.status = 'found'
      ...
      let pluginBox = `<div id='bfdc-${this.extension.site}' class='bfdc-container'>
                         ...
                         <div class='bfdc-availability'>
                            <div class='bfdc-title'>
                                <a href = 'http://greencommons.net' target='_blank'>
                                  BooksInGC
                                </a>
                            </div>
                         </div>
